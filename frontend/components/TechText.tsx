...
/* TechText from React Bits (reactbits.dev), ported to TypeScript. The container's two CSS rules are inline classes.
   Additions for a two-tone wordmark: letters from `highlightFrom` on are drawn in `highlightColor`, and
   `underline` draws a bar under that run (it follows the letters as they're dragged). */
import { useEffect, useRef } from "react";

type Settings = {
  text: string; fontFamily: string; fontWeight: number; fontSize: number; letterSpacing: number; color: string;
  accentColor: string; reach: number; softness: number; dashLength: number; dashGap: number; strokeWidth: number;
  lineStyle: "dashed" | "solid"; reveal: "area" | "letter" | "off"; specks: number; selection: boolean; labels: boolean;
  draggable: boolean; sweep: boolean; speed: number; highlightFrom: number; highlightColor: string; underline: boolean;
  align: "center" | "left";
};
type Art = { image: HTMLCanvasElement; left: number; top: number };
type Glyph = {
  char: string; x: number; box: { x1: number; y1: number; x2: number; y2: number };
  offset: { x: number; y: number }; velocity: { x: number; y: number }; outline: number; index: number;
  color: string; fill: Art; dashes: Art;
};
...
const approach = (current: number, target: number, dt: number, seconds: number) => current + (target - current) * (1 - Math.exp(-dt / seconds));
...
export default function TechText({
  ...
  className = "",
  style,
}: Partial<Settings> & { className?: string; style?: React.CSSProperties }) {
  ...
  useEffect(() => {
    settingsRef.current = {
      text, fontFamily, fontWeight, fontSize, letterSpacing, color, accentColor, reach, softness, dashLength, dashGap,
      strokeWidth, lineStyle, reveal, specks, selection, labels, draggable, sweep, speed, highlightFrom, highlightColor, underline,
      align,
    };
    wakeRef.current();
  });
  ...
  useEffect(() => {
    ...
    const sprite = (s: Settings, view: View, glyph: Pick<Glyph, "char" | "x" | "box" | "color">, stroke: boolean): Art => {
    ...
    const ensureLayout = (s: Settings): View => {
      const key = [s.text, family(s), s.fontWeight, s.fontSize, s.letterSpacing, s.color, s.dashLength, s.dashGap,
        s.strokeWidth, s.lineStyle, s.highlightFrom, s.highlightColor, s.align, width, height, dpr].join("|");
      ...
      // "left" keeps a small inset so the dashed outline and labels on the first letter are not clipped.
      const x = (s.align === "left" ? 8 : (width - inkWidth) / 2) + m.actualBoundingBoxLeft;
      ...
      chars.forEach((char, i) => {
        ...
        const base = {
          char,
          x: gx,
          color: s.highlightFrom >= 0 && i >= s.highlightFrom ? s.highlightColor : s.color,
          box: {
    ...
    const falloff = (target: CanvasRenderingContext2D, cx: number, cy: number, radius: number, strength: number, soft: number) => {
    ...
    /** The underline under the highlighted run: from its first letter's left edge to its last letter's right edge,
        a little below the baseline, each end riding its letter's drag offset. */
    const drawUnderline = (s: Settings, view: View) => {
      if (!s.underline || s.highlightFrom < 0) return;
      const run = glyphs.filter((g) => g.index >= s.highlightFrom);
      if (!run.length) return;
      const first = run[0];
      const lastG = run[run.length - 1];
      const y = view.baseline + view.size * 0.1;
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      ctx.beginPath();
      ctx.moveTo(first.box.x1 + first.offset.x, y + first.offset.y);
      ctx.lineTo(lastG.box.x2 + lastG.offset.x, y + lastG.offset.y);
      ctx.lineWidth = Math.max(2, view.size * 0.05);
      ctx.lineCap = "round";
    ...
    const tick = (now: number) => {
      ...
      glyphs.forEach((glyph, i) => {
        ...
        if (Math.abs(offset.x) < 0.05 && Math.abs(offset.y) < 0.05 && Math.hypot(velocity.x, velocity.y) < 0.5) {
          offset.x = 0; offset.y = 0; velocity.x = 0; velocity.y = 0;
          return;
        }
    ...
    const onMove = (e: PointerEvent) => { locate(e); pointer.inside = true; wake(); };
    const onLeave = () => {
  ...
  return (
    <div ref={containerRef} className={`relative size-full touch-pan-y select-none ${className}`.trim()} style={style} role="img" aria-label={text}>

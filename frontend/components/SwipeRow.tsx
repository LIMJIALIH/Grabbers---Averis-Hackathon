...
/* SwipeRow from React Bits (reactbits.dev), ported to TypeScript. Styles live in globals.css (.swipe-row).
   Changes: lucide's Trash2 replaces the Hugeicons default (no new dependency); `height` may be "auto" for rows
   whose text wraps; and with `fullSwipe` off, the first action is an ordinary button instead of always folding
   the row away (the original folds it regardless), so non-destructive drawers (Open, Copy) are possible.
   Pointer capture starts at drag, not press, so taps on links and buttons inside the row still work. */
import { useEffect, useId, useLayoutEffect, useRef, useState } from "react";
import { animate, motion, useMotionValue, useReducedMotion, useTransform } from "motion/react";
import { Trash2 } from "lucide-react";

export type SwipeAction = { id: string; label: string; icon?: React.ReactNode; color?: string; dismiss?: boolean; onSelect?: () => void };
type Grip = { id: number; x0: number; y0: number; grab: number | null; moved: boolean; hist: [number, number][]; touch: boolean };
...
/** Black or white text for a hex background. Non-hex values (CSS vars) get white. */
...
const watchWindow = (live: React.RefObject<{ move: (e: PointerEvent) => void; up: (e: PointerEvent) => void }>) => {
  const onMove = (e: PointerEvent) => { if (e.isTrusted) live.current.move(e); };
  const onUp = (e: PointerEvent) => { if (e.isTrusted) live.current.up(e); };
...
export default function SwipeRow({
  ...
  const glyphXf = useTransform([shift, landed], ([v, l]: number[]) => `translateX(${-s * l * (v - (w.current - A) / 2)}px)`);
  ...
  useEffect(() => () => { clearTimeout(foldTimer.current); unwatch.current?.(); }, []);
  ...
  const commit = (a: SwipeAction, viaKey: boolean, v = 0) => {
    ...
    const fold = () => {
      setPhase("collapsing");
      foldTimer.current = setTimeout(() => { onCommit?.(a); a.onSelect?.(); }, collapseMs);
    };
    if (viaKey || reduce) {
      if (leap) { spread.set(1); landed.set(1); }
      if (viaKey) { x.set(s * w.current); fold(); }
  ...
  const down = (e: React.PointerEvent) => {
    ...
    grip.current = { id: e.pointerId, x0: e.clientX, y0: e.clientY, grab: null, moved: false, hist: [], touch: e.pointerType === "touch" };
    // No pointer capture yet (see move): capturing on press would retarget every tap's click to the surface,
    // so buttons and links inside the row would never get theirs.
    unwatch.current?.();
  ...
  const move = (e: PointerEvent) => {
    ...
    if (g.grab === null) {
      ...
      // Capture once it's a real drag: the click that ends it then lands on the surface, not a child link.
  ...
  const up = (e: PointerEvent) => {
    ...
    try { surface.current?.releasePointerCapture(e.pointerId); } catch {}
  ...
  const act = (a: SwipeAction, e: React.MouseEvent) => {
    ...
    onAction?.(a);
    if ((a === primary && fullSwipe) || a.dismiss) {
      commit(a, e.detail === 0);
      return;
    }
    a.onSelect?.();
    ...
    setOpen(false);
    if (e.detail === 0) { heading.current = 0; x.set(0); }
    else settle(0);
  };
  const openNow = () => {
    heading.current = D;
  ...
  const onToggleKey = (e: React.KeyboardEvent) => {
    ...
    if (e.key === openKey || (toggle && !open)) { e.preventDefault(); openNow(); }
    else if (e.key === closeKey || e.key === "Escape" || (toggle && open)) { e.preventDefault(); closeNow(); }
    else if ((e.key === "Delete" || e.key === "Backspace") && open && primary && canCommit()) { e.preventDefault(); commit(primary, true); }
  };
  ...
  return (
    ...
    >
      <div className="swipe-row__clip">
        <motion.div id={railId} className="swipe-row__rail" style={{ transform: railXf }} inert={!open || undefined} aria-hidden={!open}>
          ...
          {primary ? (
            <motion.div className="swipe-row__block" style={{ transform: blockXf }}>
              <motion.button type="button" className="swipe-row__action swipe-row__action--commit" style={{ transform: glyphXf }} onClick={(e) => act(primary, e)}>
                <span className="swipe-row__glyph">
                  <span className="swipe-row__icon">{primary.icon ?? <Trash2 size={20} strokeWidth={2} />}</span>
      ...
      <span className="swipe-row__sr" aria-live="polite">{say}</span>
    </div>
  );
}
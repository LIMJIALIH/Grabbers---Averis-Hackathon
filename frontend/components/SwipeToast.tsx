...
/* SwipeToast from React Bits (reactbits.dev), ported to TypeScript. Styles live in globals.css (.swipe-toast).
   One change: lucide's X replaces the Hugeicons close glyph (no new dependency). */
import { useEffect, useRef, useState } from "react";
import { animate, motion, useMotionValue, useReducedMotion, useTransform } from "motion/react";
import { X } from "lucide-react";

export type CloseReason = "timeout" | "swipe" | "action" | "close" | "escape" | "programmatic";
...
const EXIT = 0.7;
const BURN: Keyframe[] = [{ transform: "scaleX(1)" }, { transform: "scaleX(0)" }];
...
export default function SwipeToast({
  ...
  const pendingClose = useRef<CloseReason | null>(null);
  const closeTimer = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  const reason = useRef<CloseReason>("timeout");
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    if (e.button !== 0 || !dismissible || drag.current || leaving.current || (e.target as Element).closest("button")) return;
      ...
      <div className="swipe-toast__gate">
        <div className="swipe-toast__lift">
          <motion.div
            ...
            onPointerEnter={(e) => { if (pauseOnHover && e.pointerType === "mouse") { flags.current.hover = true; syncFuse(); } }}
            onPointerLeave={(e) => { if (e.pointerType === "mouse") { flags.current.hover = false; syncFuse(); } }}
            onFocus={() => { flags.current.focus = true; syncFuse(); }}
            onBlur={(e) => {
              if (!e.currentTarget.contains(e.relatedTarget as Node | null)) { flags.current.focus = false; syncFuse(); }
            }}
            onKeyDown={(e) => {
          ...
          >
            ...
            {actionLabel ? (
              <button type="button" className="swipe-toast__action" onClick={() => { latest.current.onAction?.(); close("action"); }}>
                {actionLabel}
              </button>
            ) : null}
            {closeButton ? (

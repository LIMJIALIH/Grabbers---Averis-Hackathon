...
/* Dither from React Bits (reactbits.dev), ported to TypeScript. The container's one CSS rule is set inline. */
...
// wrapEffect's typings expect the constructor to take the props; this effect takes them as setters instead.
const WrappedRetro = wrapEffect(RetroEffectImpl) as unknown as React.ComponentType<{ colorNum: number; pixelSize: number; ref?: React.Ref<unknown> }>;

const RetroEffect = forwardRef<unknown, { colorNum: number; pixelSize: number }>(({ colorNum, pixelSize }, ref) => (
  <WrappedRetro ref={ref} colorNum={colorNum} pixelSize={pixelSize} />
));
RetroEffect.displayName = "RetroEffect";

type WaveProps = {
  waveSpeed: number; waveFrequency: number; waveAmplitude: number; waveColor: RGB; backgroundColor: RGB;
  colorNum: number; pixelSize: number; disableAnimation: boolean; enableMouseInteraction: boolean; mouseRadius: number;
};

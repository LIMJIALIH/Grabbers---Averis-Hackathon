...
/* DitherVeil from React Bits (reactbits.dev), ported to TypeScript. The container's CSS rules are inline classes. */
import { useEffect, useRef } from "react";
import { Mesh, Program, Renderer, RenderTarget, Texture, Triangle } from "ogl";

type Pattern = "bayer" | "noise" | "atkinson" | "floyd" | "lines";
type Settings = {
  fit: "contain" | "cover"; pattern: Pattern; pixelSize: number; levels: number; palette: "duotone" | "rgb";
  inkColor: string; paperColor: string; contrast: number; brightness: number; revealRadius: number; softness: number;
  linger: number; rimColor: string; rim: number; reverse: boolean; wander: boolean; clickBurst: boolean;
};
...
const KERNELS: Partial<Record<Pattern, Kernel>> = {
  atkinson: [
    [1, 0, 1 / 8],
    [2, 0, 1 / 8],
    [-1, 1, 1 / 8],
...
const measureEdge = (context: CanvasRenderingContext2D, image: HTMLImageElement) => {
  ...
  const spread = matte.reduce((total, mean, c) => total + Math.sqrt(Math.max(0, squares[c] / count - mean * mean)), 0) / 3;
...
const diffuse = (
  pixels: Uint8ClampedArray, cols: number, rows: number, kernel: Kernel, levels: number, rgb: boolean, grade: (v: number) => number,
) => {
...
export default function DitherVeil({
  ...
  className = "",
  style,
}: Partial<Settings> & { src?: string; className?: string; style?: React.CSSProperties }) {
  ...
  useEffect(() => {
    settingsRef.current = {
      fit, pattern, pixelSize, levels, palette, inkColor, paperColor, contrast, brightness, revealRadius, softness,
      linger, rimColor, rim, reverse, wander, clickBurst,
    };
    wakeRef.current();
  });
  ...
  useEffect(() => {
    ...
    const createMask = (w: number, h: number) =>
      new RenderTarget(gl, {
        ...
        height: h,
        depth: false,
        type: floatMask ? (gl as WebGL2RenderingContext).HALF_FLOAT : gl.UNSIGNED_BYTE,
        internalFormat: floatMask ? (gl as WebGL2RenderingContext).RGBA16F : gl.RGBA,
        minFilter: gl.LINEAR,
        magFilter: gl.LINEAR,
      });
  ...
  return <div ref={containerRef} className={`relative size-full touch-pan-y overflow-hidden ${className}`.trim()} style={style} />;
}
